    ...
    <div v-else>
      ...
      <form class="card auth-card" @submit.prevent="submitHandler">
        ...
        <div class="card-action">
          ...
          <p class="center">
            {{'HasAccount'|localize}}
            <router-link to="/login">{{'LoginTitle'|localize}}</router-link>
          </p>
        </div>
      </form>
    </div>
...
<script>
  ...
  export default {
    ...
    metaInfo() {
      return {title: this.$title('RegisterTitle')}
    },
    validations: {
    ...
    methods: {
      async submitHandler() {
        if (this.$v.$invalid) {
          this.$v.$touch();
          if (this.$v.agree.$dirty && !this.$v.agree.checked) {
            this.$message(localizeFilter('NeedAgree'));
          }
          return;
        }

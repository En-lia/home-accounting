  ...
  <div>
    ...
    <form class="form" v-else @submit.prevent="submitHandler">
      ...
      <div class="input-field">
        ...
        <span v-if="($v.amount.$dirty && !$v.amount.required)||( $v.amount.$dirty && !$v.amount.minValue)"
              class="helper-text invalid">
           {{'CategoriesCreate_MsgMinLength'|localize}} {{$v.amount.$params.minValue.min}}
        </span>
      </div>
      ...
      <button class="btn waves-effect waves-light" type="submit">
        {{'CategoriesCreate_Create'|localize}}
    ...
    methods: {
      async submitHandler() {
        ...
        if (this.canCreateRecord) {
          try {
            const dateSection = createDate(new Date());
            ...
            const months = this.categories.map(cat => {
              if (cat.id === this.category) return cat.months || [];
            }).filter(c => c)[0];
            if (!months.includes(dateSection)){
              months.push(dateSection);
              await this.$store.dispatch('updateCategoryMonths', {
                id: this.category,
                months
              });
            }
